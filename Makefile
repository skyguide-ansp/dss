...
.PHONY: go-lint
go-lint:
	echo "===== Checking Go lint (except for *.gen.go files) =====" && docker run --rm -v $(CURDIR):/dss -w /dss golangci/golangci-lint:v2.12.2 golangci-lint run -v
	...
	docker container run -u "$(USER_GROUP)" --rm \
	    ...
	    -v "$(CURDIR)/:/resources/src" \
			interuss/openapi-to-go-server \
				...
				--api /resources/surveillancev0.yaml#dss@surveillancev0/surveillance/v0 \
