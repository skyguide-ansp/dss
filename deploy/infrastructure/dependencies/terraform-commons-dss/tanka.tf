...
resource "local_file" "tanka_config_main" {
  content = templatefile("${path.module}/templates/main.jsonnet.tmp", {
    ...
    VAR_ENABLE_SURVEILLANCE                     = var.enable_surveillance
    ...
    VAR_DESIRED_SURVEILLANCE_DB_VERSION         = local.surveillance_db_schema
    ...
    VAR_EVICT_SURVEILLANCE_TTL                  = var.evict_surveillance_ttl
    VAR_EVICT_SURVEILLANCE_ENABLE_TSAS          = var.evict_surveillance_tsas
    VAR_EVICT_SURVEILLANCE_ENABLE_SUBSCRIPTIONS = var.evict_surveillance_subscriptions
