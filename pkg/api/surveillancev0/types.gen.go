...
// Response to a request to create or update a reference to an Traffic Surveilled Area in the DSS.
type PutTrafficSurveilledAreaResponse struct {
	// DSS subscribers that this client now has the obligation to notify of the Traffic Surveilled Area changes just made.  This client must call POST for each provided URL according to the `/uss/traffic_surveilled_areas/{id}` path API.
...
// An Traffic Surveilled Area (area in which surveillance services are being provided).  The DSS reports only these declarations and clients must exchange flight information peer-to-peer.
...
// The base URL of a USS implementation of part or all of the surveillance USS-USS API. Per the USS-USS API, the full URL
...
// Base URL for the USS's implementation of the USS API, including the POST traffic_surveilled_areas endpoint where notifications for this Subscription will be received.
